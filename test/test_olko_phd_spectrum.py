...
def test_yfy(spectrum_data_fig3p3_olko_phd: SpectrumData):
    assert np.array_equal(
        spectrum_data_fig3p3_olko_phd.bin_values_freq_times_x, [2, 4, 6]
    )
    assert np.array_equal(
        spectrum_data_fig3p3_olko_phd.bin_values_freq_times_x_normalized,
        [1 / 3, 2 / 3, 1],
    )
...
def test_dy(spectrum_data_fig3p3_olko_phd: SpectrumData):
    assert np.array_equal(
        spectrum_data_fig3p3_olko_phd.bin_values_dose_times_x, [1, 4, 9]
    )
