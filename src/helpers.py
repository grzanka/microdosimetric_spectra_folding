...
def binning_type(bin_centers: NDArray) -> SpectrumBinningType:
    ...
    if bin_centers.size >= 2 and np.allclose(
        np.diff(bin_centers), bin_centers[1] - bin_centers[0]
    ):
        result = SpectrumBinningType.linear
    # check if bin_centers form a geometric progression
    elif bin_centers.size >= 2 and np.allclose(
        np.diff(np.log(bin_centers)), np.log(bin_centers[1]) - np.log(bin_centers[0])
    ):
...
def bin_edges(bin_centers: NDArray, binning_type: SpectrumBinningType) -> NDArray:
    ...
    if binning_type == SpectrumBinningType.linear:
        ...
        result = np.append(
            bin_centers - bin_centers_diff / 2, bin_centers[-1] + bin_centers_diff / 2
        )
    if binning_type == SpectrumBinningType.log:
        ...
        result = np.append(
            bin_centers / np.sqrt(bin_centers_ratio),
            bin_centers[-1] * np.sqrt(bin_centers_ratio),
        )
    if binning_type == SpectrumBinningType.unknown and bin_centers.size > 1:
        ...
        result = np.append(
            np.append(lowest_bin_edge, middle_bin_edges), highest_bin_edge
        )
...
def others_from_freq_arrays(
    x: NDArray, freq: NDArray
...
def others_from_x_times_freq(
    x: NDArray, x_times_freq: NDArray
) -> tuple[NDArray, NDArray, NDArray]:
...
def others_from_dose_arrays(
    x: NDArray, dose: NDArray
) -> tuple[NDArray, NDArray, NDArray]:
...
def others_from_x_times_dose_arrays(
    x: NDArray, x_times_dose: NDArray
) -> tuple[NDArray, NDArray, NDArray]:
