...
class SpectrumData:
    ...
    bin_values_freq_times_x_normalized: NDArray = field(default_factory=lambda: np.empty(0))
    bin_values_dose_normalized: NDArray = field(default_factory=lambda: np.empty(0))
    bin_values_dose_times_x_normalized: NDArray = field(default_factory=lambda: np.empty(0))
    ...
    def __post_init__(self):
        ...
        # set means
        if self.bin_values_freq.size > 0:
            object.__setattr__(self, 'freq_mean', first_moment(bin_edges=self.bin_edges, bin_values=self.bin_values_freq))
        if self.bin_values_dose.size > 0:
            object.__setattr__(self, 'dose_mean', first_moment(bin_edges=self.bin_edges, bin_values=self.bin_values_dose))

        # set normalized values if bin_centers are initialized
        object.__setattr__(self, 'norm', self.bin_values_freq @ self.bin_widths)
        freq_norm = self.bin_values_freq / self.norm if self.norm != 0 else np.zeros_like(self.bin_values_freq)
        object.__setattr__(self, 'bin_values_freq_normalized', freq_norm)
        freq_times_x_norm, dose_norm, dose_times_x_norm = others_from_freq_arrays(x=self.bin_centers, freq=self.bin_values_freq_normalized)
        object.__setattr__(self, 'bin_values_freq_times_x_normalized', freq_times_x_norm)
        object.__setattr__(self, 'bin_values_dose_normalized', dose_norm)
        object.__setattr__(self, 'bin_values_dose_times_x_normalized', dose_times_x_norm)
...
def from_array(data_array: NDArray, value_type: SpectrumValueType = SpectrumValueType.freq_times_bin_centers) -> SpectrumData:
    ...
    if value_type == SpectrumValueType.freq:
        result = SpectrumData(bin_centers=data_array[:,0], bin_values_freq=data_array[:,1])
    elif value_type == SpectrumValueType.freq_times_bin_centers:
        result = SpectrumData(bin_centers=data_array[:,0], bin_values_freq_times_x=data_array[:,1])
    elif value_type == SpectrumValueType.dose_times_bin_centers:
        result = SpectrumData(bin_centers=data_array[:,0], bin_values_dose_times_x=data_array[:,1])
    return result

def from_str(data_string : str, value_type: SpectrumValueType = SpectrumValueType.freq_times_bin_centers, **kwargs) -> SpectrumData:
...
def from_csv(file_path: Path, value_type: SpectrumValueType = SpectrumValueType.freq_times_bin_centers, **kwargs) -> SpectrumData:
    '''Load spectrum from csv file. The file must contain two columns: bin_centers and bin_values_fy.'''
